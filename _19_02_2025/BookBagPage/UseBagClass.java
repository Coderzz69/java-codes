...
public class UseBagClass {
    ...
    private static void addBook() {
        ...
        Bag.Book book = bag.new Book(bookName, author);
        ...
        for (int i = 0; i < numPages; i++) {
            ...
            // Page is a private inner class, so we need a method to create pages
            book.addPage(bag.new Page(text)); // Correct way to instantiate a Page
        }
    ...
    private static void searchByKeyword() {
        ...
        Bag.Book[] found = bag.searchByKeyword(keyword);
        if (found.length == 0) {
            System.out.println("No books found containing the keyword.");
        } else {
            System.out.println("Found books:");
            for (Bag.Book book : found) {
    ...
    private static void findBookWithMostOccurrences() {
        ...
        Bag.Book book = bag.searchBookWithMostOccurrences(keyword);
        if (book != null) {
